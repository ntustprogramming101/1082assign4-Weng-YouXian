    ...
    for (int i =0;i<cabbageX.length;i++){
      for (int j =0;j<cabbageY.length;j++){
        if(i==j){
          ...
          if(cabbageX[i] * SOIL_SIZE<playerX+groundhogW&& cabbageX[i] * SOIL_SIZE+SOIL_SIZE>playerX&& (j*4+cabbageY[j]+2) * SOIL_SIZE+stroll<playerY+groundhogW&& (j*4+cabbageY[j]+2) * SOIL_SIZE+stroll+SOIL_SIZE>playerY){
            cabbageX[i]=-1;
            cabbageY[j]=-1;
            if (playerHealthMax<5){
              playerHealthMax+=1;
            }
          }
        }
